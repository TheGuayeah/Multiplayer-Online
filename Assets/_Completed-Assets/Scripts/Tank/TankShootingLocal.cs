...
namespace Complete
{
    public class TankShootingLocal : MonoBehaviour
    {
        ...
        private bool m_FireButton;                // The input axis that is used for launching shells
        ...
        public void setFireButton(InputAction.CallbackContext context)
        {
            var val = context.ReadValue<float>();
            if(val == 1) m_FireButton = true;
            else m_FireButton = false;
        }
		...
		private bool FireButton (int mode) {
			...
			switch (mode) {
			    case 0:
                    if (!m_FireAction && m_FireButton)
                    {
                        m_FireAction = true;
                        action = true;
                    }
                    else if (m_FireAction && m_FireButton)
                    {
                        action = false;
                    }					
                    break;
        ...
        private void Fire()  {
            ...
            shellInstance.GetComponent<ShellExplosionLocal>().myTankHealth = GetComponent<TankHealthLocal>();
